...
# Study 3 TOTAL - RIGHT HEMISPHERIC FRONTO-BASAL-GANGLIA FBA TRAJECTORIES

dir.create("./FBA_gams_Total/RH")

# MODELS WITH COVARS (FULL MODEL)

# GAM models for EXG3 parameters
dir.create("./FBA_gams_Total/RH/fullmodel")

models<-lapply(X=as.character(as.list(rh_dvs)),
               df=rh_xlong,
               FUN=function(dv_name, df) {
                 print(dv_name)
                 
                 # now create a new dataframe within the loop that is filtered to dv_name
                 adf<-df %>% filter(rh_dvs==dv_name) %>%
                 ...
                 save(gam1a,gam2a,gam3a,gam4a, file = paste0("./FBA_gams_Total/RH/fullmodel/", dv_name,"_gams_models.Rdata"))
                 ...
                 if (pvalue1 < 0.05) {
                   ...
                   ggsave(filename=paste0("./FBA_gams_Total/RH/fullmodel/", dv_name, '.png'), plot=best_plot,width=6,height=5)
                   
                   tiff(paste0("./FBA_gams_Total/RH/fullmodel/", dv_name, "_check.tiff"), width = 9, height = 6, units = 'in', res = 150)
                   par(mfrow=c(2,2))
...
write.csv(RH_FBA_models_EXG3,file= "./FBA_gams_Total/RH/fullmodel/rh_fba_models.csv",row.names=F)
...
write_csv(RH_modelsFDR,path = "./FBA_gams_Total/RH/fullmodel/rh_fba_models_FDR.csv")
