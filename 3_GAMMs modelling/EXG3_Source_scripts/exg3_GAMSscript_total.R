...
dir.create("./Total_sample/output/")

# GAM models for EXG3 parameters
dir.create("./Total_sample/output/fullmodel")
...
models<-lapply(X=as.character(as.list(dvstotal)),
               ...
               FUN=function(dv_name, df) {
                 ...
                 save(gam1a,gam2a,gam3a,gam4a, file = paste0("./Total_sample/output/fullmodel/", dv_name,"_gams_models.Rdata"))
                 ...
                 if (pvalue1 < 0.05) {
                   if (pvalue2 < 0.05) {
                     if (pvalue3 < 0.05) {
                       ...
                       best_plot <- ggplot() +
                         ...
                         geom_ribbon(data=pred3a,aes(x=age,ymin=lower,ymax=upper,colour=OFgroup,fill=OFgroup),alpha=0.1)+
                     ...
                     best_plot <- ggplot() +
                       ...
                       geom_ribbon(data=pred2a,aes(x=age,ymin=lower,ymax=upper,colour=OFgroup,fill=OFgroup),alpha=0.1)+
                       scale_fill_manual(name = "Fitted model", values=c("#F8766D","#00BFC4"), labels = c("ADHD", "CONTROL")) +
                       geom_point(data=adf,aes(x=age,y=value,group=SID,colour=OFgroup),size=2,alpha=0.3) +
                       geom_line(data=adf,aes(x=age,y=value,group=SID,colour=OFgroup),size=.3,alpha=0.3) +
                       scale_colour_manual(name = "Raw data", values=c("#F8766D","#00BFC4"), labels = c("ADHD", "CONTROL")) +
                   ...
                   ggsave(filename=paste0("./Total_sample/output/fullmodel/", dv_name, '.png'), plot=best_plot,width=6,height=5)
                   
                   tiff(paste0("./Total_sample/output/fullmodel/", dv_name, "_check.tiff"), width = 9, height = 6, units = 'in', res = 150)
                   par(mfrow=c(2,2))
...
write.csv(total_models_EXG3,file= "./Total_sample/output/fullmodel/total_EXG3_models.csv",row.names=F)
